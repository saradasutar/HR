...
const CONFIG = Object.freeze({
  ...
  SIDEBAR_COLLAPSED_STORAGE_KEY = "hrDashboardSidebarCollapsedV184",
  ...
  SESSION_ALLOWED_TIMEOUT_MINUTES = Object.freeze([5, 10, 15, 20, 30, 60, 120]),
...
function init() {
  ...
  }, 120)), refs.stickyNoteDetails.addEventListener("input", autoFitStickyDetailsInput), refs.sidebarToggle.addEventListener("click", toggleSidebar), refs.sidebarBackdrop.addEventListener("click", closeMobileSidebar), refs.sidebarHomeButton.addEventListener("click", scrollToEmployeeDirectory), refs.dashboardSidebar.querySelectorAll(".sidebar-nav-item").forEach(e => e.addEventListener("click", () => {
  ...
  }, 150)), refs.workDiaryButton.addEventListener("click", openWorkDiary), refs.sidebarQuickDiaryForm.addEventListener("submit", saveSidebarQuickDiaryEntry), refs.sidebarQuickDiaryInput.addEventListener("keydown", e => {
    (e.metaKey || e.ctrlKey) && "Enter" === e.key && (e.preventDefault(), refs.sidebarQuickDiaryForm.requestSubmit ? refs.sidebarQuickDiaryForm.requestSubmit() : saveSidebarQuickDiaryEntry(new Event("submit", {
      cancelable: !0
    })))
...
function formatSessionTimeoutLabel(e) {
  return e >= 60 && e % 60 === 0 ? `${e/60} hour${e>60?"s":""}` : `${e} minutes`
}
...
function updateSessionSecurityText() {
  const e = normalizeSessionTimeoutMinutes(state.sessionTimeoutMinutes),
    t = formatSessionTimeoutLabel(e);
  refs.loginSessionSecurityNote && (refs.loginSessionSecurityNote.textContent = `🔐 Automatic sign-out after ${t} of inactivity or after leaving/closing the dashboard.`), refs.currentSessionTimeout && (refs.currentSessionTimeout.textContent = t), refs.sessionTimeoutMinutes && (refs.sessionTimeoutMinutes.value = String(e))
}
...
function checkSessionActivity() {
  ...
  const e = Number(sessionStorage.getItem(SESSION_LAST_ACTIVITY_KEY) || 0);
  !e || Date.now() - e >= sessionIdleTimeoutMs() ? performLogout(`You were signed out automatically after ${formatSessionTimeoutLabel(normalizeSessionTimeoutMinutes(state.sessionTimeoutMinutes))} of inactivity.`) : armSessionIdleTimer()
}
...
function updateSessionWarningCountdown() {
  ...
  refs.sessionWarningCountdown.textContent = `${Math.floor(t/60)}:${r}`, refs.sessionWarningMessage.textContent = `Your ${formatSessionTimeoutLabel(normalizeSessionTimeoutMinutes(state.sessionTimeoutMinutes))} inactivity limit is almost reached.`, t <= 0 && checkSessionActivity()
}
...
function updateReportHeaderFooterDisplay() {
  ...
  refs.reportPrintFooter.hidden = !o, refs.reportPrintFooter.classList.toggle("report-footer-repeat", s), saveReportHeaderFooterPrefs(), refs.reportOrientationLandscape && applyPrintOrientation()
}
...
function applyPrintOrientation() {
  const e = currentReportOrientation(),
    t = readReportPageSizePref(),
    r = Boolean(refs.reportPrintFooter && !refs.reportPrintFooter.hidden && refs.reportFooterRepeat && refs.reportFooterRepeat.checked),
    s = r ? "10mm 10mm 24mm 10mm" : "10mm";
  let o = document.getElementById("printOrientationStyle");
  o || (o = document.createElement("style"), o.id = "printOrientationStyle", document.head.appendChild(o)), o.textContent = `@media print{@page{size:${t} ${e};margin:${s}}}`
}

function readReportPageSizePref() {
  try {
    const e = localStorage.getItem(REPORT_PAGE_SIZE_STORAGE_KEY);
    return ["A4", "Letter", "Legal"].includes(e) ? e : "A4"
  } catch {
    return "A4"
  }
}

function saveReportPageSizePref(e) {
  try {
    localStorage.setItem(REPORT_PAGE_SIZE_STORAGE_KEY, e)
  } catch {}
}

function handleReportPageSizeChange() {
  saveReportPageSizePref(refs.reportPageSize.value), applyPrintOrientation()
}
...
function initReportOrientationField() {
  ...
  refs.reportOrientationLandscape.checked = "landscape" === e, refs.reportOrientationPortrait.checked = "portrait" === e, refs.reportPageSize && (refs.reportPageSize.value = readReportPageSizePref()), applyPrintOrientation()
}
...
function reportColumns(e, t) {
  ...
  if (Array.isArray(state.reportPrintColumns)) {
    if (0 === state.reportPrintColumns.length) return [];
...
function updateReportPrintColumnsSummary(e) {
  ...
  const t = e || reportColumnCatalog(refs.reportType.value, parseDate(refs.reportReferenceDate.value) || new Date),
    ...
    s = t.length,
    o = Array.isArray(state.reportPrintColumns) && 0 === state.reportPrintColumns.length;
  refs.reportPrintColumnsSummary.textContent = o ? "No columns selected — choose at least one to print" : Array.isArray(state.reportPrintColumns) ? `${r} of ${s} columns selected` : `Using default columns (${r})`, refs.reportPrintColumnsSummary.classList.toggle("is-warning", o)
}
...
function handleReportPrintColumnsChange() {
  ...
  state.reportPrintColumns = e, updateReportPrintColumnsSummary(), generateReport()
}
...
function deselectAllReportPrintColumns() {
  state.reportPrintColumns = [], renderReportPrintColumnsOptions(), generateReport()
}
...
function scheduleLeftPanelCountsLoad() {
  clearTimeout(state.leftPanelCountsLoadTimer), state.leftPanelCountsLoadTimer = window.setTimeout(() => {
    state.leftPanelCountsLoadTimer = 0, loadLeftPanelCounts().catch(() => {})
  }, 280)
}
...
function populateDiaryFilters() {
  refs.workDiaryCount && (refs.workDiaryCount.textContent = String(state.diaryEntries.length));
  const e = [...new Set(state.diaryEntries.map(e => String(e.entryDate || "").slice(0, 7)).filter(Boolean))].sort().reverse(),
    y = [...new Set(state.diaryEntries.map(e => String(e.entryDate || "").slice(0, 4)).filter(Boolean))].sort().reverse(),
    ...
    y0 = refs.diaryYearFilter.value,
    o = refs.diaryCategoryFilter.value,
    i = refs.diarySourceFilter.value;
  refs.diaryMonthFilter.innerHTML = '<option value="">All months</option>' + e.map(e => `<option value="${escapeAttribute(e)}">${escapeHtml(new Date(`${e}-01T00:00:00`).toLocaleDateString("en-IN",{month:"long",year:"numeric"}))}</option>`).join(""), refs.diaryYearFilter.innerHTML = '<option value="">All years</option>' + y.map(e => `<option value="${escapeAttribute(e)}">${escapeHtml(e)}</option>`).join(""), refs.diaryCategoryFilter.innerHTML = '<option value="">All categories</option>' + t.map(e => `<option>${escapeHtml(e)}</option>`).join(""), refs.diarySourceFilter.innerHTML = '<option value="">All sources</option>' + r.map(e => `<option>${escapeHtml(e)}</option>`).join(""), e.includes(s) && (refs.diaryMonthFilter.value = s), y.includes(y0) && (refs.diaryYearFilter.value = y0), t.includes(o) && (refs.diaryCategoryFilter.value = o), r.includes(i) && (refs.diarySourceFilter.value = i)
}

function diaryFilteredEntries() {
  const e = refs.diarySearch.value.trim().toLowerCase(),
    t = refs.diaryMonthFilter.value,
    y = refs.diaryYearFilter.value,
    r = refs.diaryCategoryFilter.value,
    s = refs.diarySourceFilter.value;
  return state.diaryEntries.filter(o => !(t && !String(o.entryDate).startsWith(t)) && !(y && !String(o.entryDate).startsWith(y)) && ((!r || o.category === r) && ((!s || o.source === s) && (!("learning" === state.diaryView && !o.learning) && !(e && ![o.title, o.details, o.category, o.tags, o.relatedEmployee, o.source, o.createdBy].join(" ").toLowerCase().includes(e))))))
}

function renderDiaryEntries() {
  document.querySelectorAll("[data-diary-view]").forEach(e => e.classList.toggle("active", e.dataset.diaryView === state.diaryView));
  const isCal = "calendar" === state.diaryView;
  if (refs.diaryCalendarWrap && (refs.diaryCalendarWrap.hidden = !isCal), refs.diaryEntryList.hidden = isCal, isCal) return refs.diaryEmptyState.hidden = !0, refs.diaryLearningCount.textContent = `${state.diaryEntries.filter(e=>e.learning).length} learnings`, refs.diaryResultSummary.textContent = "Browse by date", void renderDiaryCalendar();
...
function renderDiaryCalendar() {
  if (!refs.diaryCalendarGrid) return;
  const [e, t] = state.diaryCalendarMonth.split("-").map(Number),
    r = new Date(e, t - 1, 1),
    s = new Date(e, t, 0).getDate(),
    o = r.getDay(),
    i = {};
  state.diaryEntries.forEach(e => {
    e.entryDate && (i[e.entryDate] || (i[e.entryDate] = [])).push(e)
  }), refs.diaryCalMonthLabel.textContent = r.toLocaleDateString("en-IN", {
    month: "long",
    year: "numeric"
  });
  const n = isoToday(),
    a = [];
  for (let r = 0; r < o; r++) a.push('<span class="diary-cal-cell empty"></span>');
  for (let o = 1; o <= s; o++) {
    const l = `${e}-${String(t).padStart(2,"0")}-${String(o).padStart(2,"0")}`,
      d = (i[l] || []).length,
      c = l === n,
      u = l === state.diaryCalendarSelectedDate;
    a.push(`<button type="button" class="diary-cal-cell${d?" has-entries":""}${c?" is-today":""}${u?" is-selected":""}" data-cal-date="${l}">${o}${d?`<b>${d}</b>`:""}</button>`)
  }
  refs.diaryCalendarGrid.innerHTML = a.join(""), renderDiaryCalendarDayEntries()
}

function renderDiaryCalendarDayEntries() {
  if (!state.diaryCalendarSelectedDate) return void(refs.diaryCalendarDayEntries.innerHTML = '<p class="sticky-empty">Click a date above to see that day\'s entries.</p>');
  const e = state.diaryEntries.filter(e => e.entryDate === state.diaryCalendarSelectedDate);
  refs.diaryCalendarDayEntries.innerHTML = e.length ? `<h4>${escapeHtml(formatDate(state.diaryCalendarSelectedDate))} · ${e.length} entr${1===e.length?"y":"ies"}</h4>${e.map(diaryEntryMarkup).join("")}` : `<p class="sticky-empty">No entries on ${escapeHtml(formatDate(state.diaryCalendarSelectedDate))}.</p>`
}

function changeDiaryCalendarMonth(e) {
  const [t, r] = state.diaryCalendarMonth.split("-").map(Number),
    s = new Date(t, r - 1 + e, 1);
  state.diaryCalendarMonth = `${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`, renderDiaryCalendar()
}
...
function highlightDiaryText(e, t) {
  const r = escapeHtml(e || "");
  if (!t) return r;
  const s = escapeHtml(t).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return r.replace(new RegExp(`(${s})`, "gi"), '<mark class="diary-search-hit">$1</mark>')
}

function diaryEntryMarkup(e) {
  const t = "admin" === state.role || e.createdBy === state.username,
    n = refs.diarySearch ? refs.diarySearch.value.trim() : "",
    i = "Personal" === e.category,
    r = `${e.learning?'<span class="diary-badge learning">💡 Learning</span>':""}${i?'<span class="diary-badge personal">🏠 Personal</span>':""}${e.important?'<span class="diary-badge important">★ Important</span>':""}<span class="diary-badge source">${escapeHtml(e.source||"Manual")}</span>`,
    a = e.details && e.details !== e.title;
  return `<article class="diary-card${e.important?" is-important":""}${e.learning?" is-learning-entry":""}${i?" is-personal-entry":""}"><header><time>${escapeHtml(formatDate(e.entryDate))}</time><div>${r}</div></header><h4>${highlightDiaryText(e.title,n)}</h4>${a?`<p>${highlightDiaryText(e.details,n)}</p>`:""}<div class="diary-meta">${e.category?`<span>${escapeHtml(e.category)}</span>`:""}${e.tags?`<span># ${escapeHtml(e.tags)}</span>`:""}${e.relatedEmployee?`<span>👤 ${escapeHtml(e.relatedEmployee)}</span>`:""}${e.createdBy?`<span>By ${escapeHtml(e.createdBy)}</span>`:""}</div><footer>${e.link?`<a href="${escapeAttribute(e.link)}" target="_blank" rel="noopener">🔗 Open link ↗</a>`:"<span></span>"}${t?`<div><button type="button" data-edit-diary="${escapeAttribute(e.id)}">Edit</button><button type="button" class="delete" data-delete-diary="${escapeAttribute(e.id)}">Delete</button></div>`:""}</footer></article>`
}
...
function resetDiaryForm() {
  state.diaryEditId = "", refs.diaryEntryForm.reset(), refs.diaryEntryDate.value = (new Date).toISOString().slice(0, 10), refs.diaryEntryVisibility.value = "Private", refs.diaryFormHeading.textContent = "New diary entry", refs.cancelDiaryEditButton.hidden = !0, refs.diaryEntryError.textContent = "", updateDiaryTitleCounter(), autoGrowDiaryDetails(), refs.workDiaryDialog.open && setTimeout(() => refs.diaryEntryTitle.focus(), 30)
}

function updateDiaryTitleCounter() {
  refs.diaryTitleCounter && (refs.diaryTitleCounter.textContent = `${refs.diaryEntryTitle.value.length}/140`)
}

function autoGrowDiaryDetails() {
  refs.diaryEntryDetails && (refs.diaryEntryDetails.style.height = "auto", refs.diaryEntryDetails.style.height = `${Math.min(420, Math.max(96, refs.diaryEntryDetails.scrollHeight))}px`)
}

function handleDiaryFormShortcut(e) {
  (e.metaKey || e.ctrlKey) && "Enter" === e.key && (e.preventDefault(), refs.diaryEntryForm.requestSubmit ? refs.diaryEntryForm.requestSubmit() : saveDiaryEntry(new Event("submit", {
    cancelable: !0
  })))
}

function handleDiarySearchInput() {
  refs.diarySearchClear && (refs.diarySearchClear.hidden = !refs.diarySearch.value)
}

function clearDiarySearchOnly() {
  refs.diarySearch.value = "", refs.diarySearchClear.hidden = !0, refs.diarySearch.focus(), renderDiaryEntries()
}

function readSidebarCollapsedPref() {
  try {
    return "1" === localStorage.getItem(SIDEBAR_COLLAPSED_STORAGE_KEY)
  } catch {
    return !1
  }
}

function saveSidebarCollapsedPref(e) {
  try {
    localStorage.setItem(SIDEBAR_COLLAPSED_STORAGE_KEY, e ? "1" : "0")
  } catch {}
}
...
function initSidebarState() {
  if (!refs.dashboardSidebar) return;
  isMobileSidebarWidth() ? (refs.dashboardSidebar.classList.remove("collapsed"), refs.sidebarToggle.setAttribute("aria-expanded", "false")) : (refs.dashboardSidebar.classList.toggle("collapsed", readSidebarCollapsedPref()), refs.sidebarToggle.setAttribute("aria-expanded", String(!readSidebarCollapsedPref())))
}

function closeMobileSidebar() {
  refs.dashboardSidebar.classList.remove("mobile-open"), refs.sidebarBackdrop.hidden = !0, refs.sidebarToggle.setAttribute("aria-expanded", "false")
}

function toggleSidebar() {
  if (isMobileSidebarWidth()) {
    const e = refs.dashboardSidebar.classList.toggle("mobile-open");
    refs.sidebarBackdrop.hidden = !e, refs.sidebarToggle.setAttribute("aria-expanded", String(e))
  } else {
    const e = refs.dashboardSidebar.classList.toggle("collapsed");
    refs.sidebarToggle.setAttribute("aria-expanded", String(!e)), saveSidebarCollapsedPref(e)
  }
}

function scrollToEmployeeDirectory() {
  const e = document.querySelector(".data-panel");
  e && e.scrollIntoView({
    behavior: "smooth",
    block: "start"
  }), isMobileSidebarWidth() && closeMobileSidebar()
...
function renderSidebarQuickDiaryList() {
  ...
  refs.sidebarQuickDiaryList.innerHTML = r.map(e => `<button type="button" class="quick-diary-recent-item${e.learning?" is-learning":""}${"Personal"===e.category?" is-personal":""}" data-quick-diary-recent-open="${escapeAttribute(e.id)}"><span class="quick-diary-recent-header">${e.learning?'<span class="quick-diary-recent-learning-badge" title="Marked as Learning">💡</span>':"Personal"===e.category?'<span class="quick-diary-recent-learning-badge" title="Personal">🏠</span>':""}<span class="quick-diary-recent-date">${escapeHtml(formatDate(e.entryDate))}</span>${e.link?'<span class="quick-diary-recent-link" title="Has a link">🔗</span>':""}</span><span class="quick-diary-recent-title">${escapeHtml(e.title||"Untitled")}</span></button>`).join(""), refs.sidebarQuickDiaryEmpty.hidden = r.length > 0, refs.sidebarQuickDiaryEmpty.textContent = e ? "No matching entries." : "No entries yet."
}
...
function deriveQuickDiaryTitle(e) {
  if (e.length <= 60) return e;
  const t = e.slice(0, 60),
    r = t.lastIndexOf(" ");
  return (r > 20 ? t.slice(0, r) : t) + "…"
}
...
async function saveSidebarQuickDiaryEntry(e) {
  ...
  if (!t) return;
  const n = (refs.sidebarQuickDiaryForm.querySelector('input[name="sidebarQuickDiaryCategory"]:checked') || {}).value || "Work",
    r = {
    id: "",
    entryDate: isoToday(),
    title: deriveQuickDiaryTitle(t),
    details: t,
    category: n,
    tags: "",
    relatedEmployee: "",
    link: refs.sidebarQuickDiaryLink.value.trim(),
    visibility: "Private",
    learning: "Learning" === n,
    important: !1
  ...
  try {
    ...
    state.diaryEntries = e.entries || [], refs.sidebarQuickDiaryInput.value = "", refs.sidebarQuickDiaryLink.value = "", refs.sidebarQuickDiaryForm.querySelector('input[value="Work"]').checked = !0, refs.workDiaryCount && (refs.workDiaryCount.textContent = String(state.diaryEntries.length)), refs.workDiaryDialog.open && (populateDiaryFilters(), renderDiaryEntries()), renderSidebarQuickDiaryList(), refs.sidebarQuickDiaryInput.focus()
  } catch (e) {
...
function clearDiaryFilters() {
  refs.diarySearch.value = "", refs.diaryMonthFilter.value = "", refs.diaryYearFilter.value = "", refs.diaryCategoryFilter.value = "", refs.diarySourceFilter.value = "", state.diaryView = "recent", renderDiaryEntries()
}

function printDiaryEntries() {
  let e, n;
  if ("calendar" === state.diaryView) {
    if (state.diaryCalendarSelectedDate) e = state.diaryEntries.filter(e => e.entryDate === state.diaryCalendarSelectedDate), n = [formatDate(state.diaryCalendarSelectedDate)];
    else {
      const t = state.diaryCalendarMonth;
      e = state.diaryEntries.filter(e => String(e.entryDate).startsWith(t)), n = [new Date(`${t}-01T00:00:00`).toLocaleDateString("en-IN", {
        month: "long",
        year: "numeric"
      })]
    }
  } else e = diaryFilteredEntries(), n = [];
  if (!e.length) return void showToast("No diary entries match the current view.", !0);
  if ("calendar" !== state.diaryView) {
    const t = refs.diaryMonthFilter.value,
      r = refs.diaryYearFilter.value,
      s = refs.diaryCategoryFilter.value,
      o = refs.diarySourceFilter.value,
      i = refs.diarySearch.value.trim();
    t ? n.push(new Date(`${t}-01T00:00:00`).toLocaleDateString("en-IN", {
      month: "long",
      year: "numeric"
    })) : r && n.push(`Year ${r}`), s && n.push(s), o && n.push(o), i && n.push(`Search: "${i}"`)
  }
  refs.diaryPrintTitle.textContent = "Work Diary" + (n.length ? ` — ${n.join(" · ")}` : ""), refs.diaryPrintMeta.textContent = `${e.length} entr${1===e.length?"y":"ies"} · Printed ${new Intl.DateTimeFormat("en-IN",{dateStyle:"medium",timeStyle:"short"}).format(new Date)}`, refs.diaryPrintList.innerHTML = e.map(e => `<article class="diary-print-card"><header><strong>${escapeHtml(formatDate(e.entryDate))}</strong><span>${escapeHtml(e.title||"")}</span></header>${e.details?`<p>${escapeHtml(e.details)}</p>`:""}<footer>${e.category?`<span>${escapeHtml(e.category)}</span>`:""}${e.tags?`<span># ${escapeHtml(e.tags)}</span>`:""}${e.relatedEmployee?`<span>👤 ${escapeHtml(e.relatedEmployee)}</span>`:""}</footer></article>`).join(""), document.body.classList.add("printing-diary"), window.print()
}
...
function renderFileRecords() {
  refs.fileRegisterCount && (refs.fileRegisterCount.textContent = String(state.fileRecords.length));
