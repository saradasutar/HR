...
function renderTable() {
  ...
  refs.tableBody.innerHTML = pageRows.map((employee, rowIndex) => {
    ...
    const cells = tableColumns.map((column) => {
      ...
      if (!emptyValue && visualClass === "employee-name-column") decoratedValue = `<span class="metallic-text metallic-sapphire">${value}</span>`;
...
function detailRow(field, rawValue) {
  ...
  if (raw && visualClass === "employee-name-column") value = `<span class="metallic-text metallic-sapphire">${value}</span>`;
