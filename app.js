...
function renderSidebarQuickDiaryList() {
  ...
  refs.sidebarQuickDiaryList.innerHTML = r.map(e => `<button type="button" class="quick-diary-recent-item${e.learning?" is-learning":""}" data-quick-diary-recent-open="${escapeAttribute(e.id)}">${e.learning?'<span class="quick-diary-recent-learning-badge" title="Marked as Learning">💡</span>':""}<span class="quick-diary-recent-date">${escapeHtml(formatDate(e.entryDate))}</span><span class="quick-diary-recent-title">${escapeHtml(e.title||"Untitled")}</span></button>`).join(""), refs.sidebarQuickDiaryEmpty.hidden = r.length > 0, refs.sidebarQuickDiaryEmpty.textContent = e ? "No matching entries." : "No entries yet."
}
...
async function saveSidebarQuickDiaryEntry(e) {
  ...
  const r = {
    ...
    title: t.slice(0, 140),
    details: t,
    category: refs.sidebarQuickDiaryLearning.checked ? "Learning" : "Work",
    tags: "",
    relatedEmployee: "",
    link: "",
    visibility: "Private",
    learning: refs.sidebarQuickDiaryLearning.checked,
    important: !1
  }, s = refs.sidebarQuickDiaryForm.querySelector('button[type="submit"]');
  setButtonBusy(s, !0, "…");
  try {
    const e = await apiRequest("saveDiaryEntry", r);
    state.diaryEntries = e.entries || [], refs.sidebarQuickDiaryInput.value = "", refs.sidebarQuickDiaryLearning.checked = !1, refs.workDiaryCount && (refs.workDiaryCount.textContent = String(state.diaryEntries.length)), refs.workDiaryDialog.open && (populateDiaryFilters(), renderDiaryEntries()), renderSidebarQuickDiaryList(), refs.sidebarQuickDiaryInput.focus()
  } catch (e) {
...
function printDiaryEntries() {
  const e = diaryFilteredEntries();
  if (!e.length) return void showToast("No diary entries match the current filters.", !0);
